...
class UnexpectedRootValidationPlugin(GlobalValidationPlugin):
    """A resource-scoped validation plugin validating that no root is specified in
    a datapack if the schemapack is unrooted.
    This plugin only applies to unrooted schemapacks.
    """
    ...
    def validate(self, *, datapack: DataPack):
        ...
        if datapack.root:
            raise ValidationPluginError(
                type_="UnexpectedRootError",
                message="Unexpected root in datapack for unrooted schemapack.",
            )
