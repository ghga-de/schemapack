...
class MissingRootValidationPlugin(GlobalValidationPlugin):
    """A resource-scoped validation plugin validating that a root is specified in
    a datapack if the schemapack is rooted.
    ...
    def validate(self, *, datapack: DataPack):
        ...
        if not datapack.root:
            raise ValidationPluginError(
                type_="MissingRootError",
                message="Missing root in datapack for rooted schemapack.",
            )
